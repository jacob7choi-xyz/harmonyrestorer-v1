...
class TestChunkAudio:
    """Tests for chunk_audio frame-splitting logic."""

    def test_short_audio_returns_single_padded_chunk(self) -> None:
        """Audio shorter than FRAME_LEN produces one zero-padded chunk."""
    ...
    def test_exact_frame_length_returns_single_chunk(self) -> None:
        """Audio exactly FRAME_LEN long produces one chunk with no padding."""
        audio = np.random.default_rng(42).standard_normal(FRAME_LEN).astype(np.float32)
        chunks = chunk_audio(audio)
    ...
    def test_two_frames_exactly(self) -> None:
        """Audio of length FRAME_LEN + _STEP produces exactly two chunks."""
        length = FRAME_LEN + _STEP
    ...
    def test_overlap_between_consecutive_chunks(self) -> None:
        """Consecutive chunks overlap by exactly OVERLAP samples."""
        length = FRAME_LEN + 2 * _STEP
        audio = np.ones(length, dtype=np.float32)
        chunks = chunk_audio(audio)
        ...
        for i in range(len(chunks) - 1):
            ...
            end_a = start_a + FRAME_LEN
            overlap = end_a - start_b
            assert overlap == OVERLAP

    def test_short_tail_is_dropped(self) -> None:
        """A tail shorter than OVERLAP is dropped (covered by previous chunk)."""
        # Two full chunks fit at starts 0 and _STEP. The next start is 2*_STEP.
        # remaining = length - 2*_STEP must be <= OVERLAP for the tail to be dropped.
        length = 2 * _STEP + OVERLAP
        audio = np.ones(length, dtype=np.float32)
        chunks = chunk_audio(audio)
    ...
    def test_tail_longer_than_overlap_gets_padded_chunk(self) -> None:
        """A tail longer than OVERLAP gets its own zero-padded chunk."""
        length = FRAME_LEN + _STEP + OVERLAP + 100
        audio = np.ones(length, dtype=np.float32)
        chunks = chunk_audio(audio)

        assert len(chunks) == 3
        # Last chunk should be zero-padded to FRAME_LEN
        _, last_frame = chunks[-1]
        assert len(last_frame) == FRAME_LEN
    ...
    def test_all_chunks_have_correct_length(self) -> None:
        """Every chunk is exactly FRAME_LEN samples, even for long audio."""
        length = FRAME_LEN * 10 + 5000
...
class TestOverlapAdd:
    """Tests for overlap_add reassembly logic."""
    ...
    def test_single_chunk_returns_trimmed(self) -> None:
        ...
        result = overlap_add([(0, frame)], original_length)
    ...
    def test_single_chunk_returns_copy(self) -> None:
        ...
        frame = np.ones(FRAME_LEN, dtype=np.float32)
        result = overlap_add([(0, frame)], 100)
        result[0] = 999.0
        assert frame[0] == 1.0
    ...
    def test_last_chunk_no_fade_out(self) -> None:
        ...
        np.testing.assert_allclose(result[FRAME_LEN:], 1.0, atol=1e-6)
